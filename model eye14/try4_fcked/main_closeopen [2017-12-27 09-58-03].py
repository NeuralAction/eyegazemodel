...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #conv
    h_pool = conv2dSingle(x, phase_train, useBnorm, [3, 3, 16], stride = 2, poolsize = 1)
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 32], poolsize = 1)
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 64])
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 64])
    h_pool = avg_pool_2x2(h_pool, 4)
    print(h_pool)
    h_pool = flat(h_pool)
    print(h_pool)
        ...
        for i in range(0, int(data.size * ephoc / batch_size)):
            ...
            if i !=0 and i % debugstep == 0:
                ...
                if(e!=lastephoc):
                    ...
                    plt.plot(acc_steps, acc_test, label="Train")
                    plt.ylim([0,1])
                    plt.xlabel("step")
                    plt.ylabel("accuracy")
...
if __name__ == "__main__":
    ...
    basedir = "C:\\Library\\정올 2017\\Source\\NeuralAction\\Vision\\EyeExtract\\bin\\x64\\Debug\\Vision\\"
    ...
    Train(True, loadmodel=False, ep=60, useBnorm=True, bsize=20, tbsize=20, debugStep=100, decayEphoc=4)
    FreezeGraph()

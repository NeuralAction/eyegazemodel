...
def conv2dSingle(pool, phase_train, useBnorm, weightShape, stride = 1, poolsize=2, afunc=activate):
...
def depthConv2dSingle(pool, phase_train, useBnorm, kernelShape, chMul = 1, stride = 2, afunc=activate):
...
mobileLayerConvertToConv = False
def mobileLayer(pool, phase_train, useBnorm, weightShape, chMul = 1, poolsize = 2, afunc=activate):
    if(mobileLayerConvertToConv):
        pool = conv2dSingle(pool, phase_train, useBnorm, weightShape, stride = 1, poolsize = poolsize, afunc = afunc)
    else:
        pool = depthConv2dSingle(pool, phase_train, useBnorm, [weightShape[0], weightShape[1]], chMul, poolsize, afunc=afunc)
        pool = conv2dSingle(pool, phase_train, useBnorm, [1, 1, weightShape[2]], 1, 1, afunc=afunc)
    return pool
...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 96], poolsize = 1)
    h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 96])
    h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 128], poolsize = 1)
    ...
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 128], poolsize = 1, afunc=None)
    f_pool_res = f_pool
    ...
    for i in range(1):
        ...
        f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 128], poolsize = 1, afunc=None)
        f_pool_res_t = f_pool
        #f_pool = f_pool_res + f_pool
        f_pool_res = f_pool_res_t
        f_pool = activate(f_pool)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 256], poolsize = 1, afunc=None)
    f_pool_res = f_pool
    f_pool = activate(f_pool)
    for i in range(1):
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eye14\\deeper\\"
    modeltitle = "face2"
    #opt options
    mobileLayerConvertToConv = True
    useSELU = True
    useBnorm = False

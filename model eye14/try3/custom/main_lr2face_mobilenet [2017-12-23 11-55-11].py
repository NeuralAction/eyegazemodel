...
def conv2dSingle(pool, phase_train, useBnorm, weightShape, stride = 1, poolsize=2, afun=activate):
...
def depthConv2dSingle(pool, phase_train, useBnorm, kernelShape, chMul = 1, stride = 2, afun=activate):
...
def mobileLayer(pool, phase_train, useBnorm, weightShape, chMul = 1, poolsize = 2, afun=activate):
    pool = depthConv2dSingle(pool, phase_train, useBnorm, [weightShape[0], weightShape[1]], chMul, poolsize, afun=afun)
    pool = conv2dSingle(pool, phase_train, useBnorm, [1, 1, weightShape[2]], 1, 1, afun=afun)
    return pool
...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    # h_pool, h_poolR = conv2dShared(x_l, x_r, phase_train, useBnorm, [3, 3, 16], poolsize = 1, stride = 2) 
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 24], chMul = 2, poolsize = 1)
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 32], chMul = 2)
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 48], chMul = 2, poolsize = 1)
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 48], poolsize = 1)
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 48])
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 64], chMul = 2, poolsize = 1)
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 64], poolsize = 1)
    # h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 64])
    ...
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 128], chMul = 2, poolsize = 1, afun=None)
    f_pool_res = f_pool
    f_pool = activate(f_pool)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 128], poolsize = 1)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 128], poolsize = 1, afun=None)
    f_pool = f_pool + f_pool_res
    f_pool = activate(f_pool)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 256], chMul = 2, poolsize=1, afun=None)
    f_pool_res = f_pool
    f_pool = activate(f_pool)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 256], poolsize = 1)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 256], poolsize = 1, afun=None)
    f_pool = f_pool + f_pool_res
    f_pool = activate(f_pool)
    #really can i use this?
    f_pool = max_pool(f_pool)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 386], chMul = 2, poolsize=1, afun=None)
    f_pool_res = f_pool
    f_pool = activate(f_pool)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 386], poolsize = 1)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 386], poolsize = 1, afun=None)
    f_pool = f_pool + f_pool_res
    f_pool = activate(f_pool)
    f_pool = flat(f_pool)
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eye14\\custom\\"
    modeltitle = "face2"
    #opt options
    useSELU = True
    useBnorm = False

...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #eye
    # h_pool, h_poolR = conv2dShared(x_l, x_r, phase_train, useBnorm, [3, 3, 16], poolsize=1)
    ...
    h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 64], poolsize = 1)
    h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 64], poolsize = 2)
    ...
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 386], poolsize = 1)
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 386], poolsize = 2)
    f_pool = flat(f_pool)
    #f_pool = dropout(f_pool, keep_prob, phase_train)
    f_pool = fc(f_pool, 256, keep_prob, phase_train)
    print(f_pool)
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eye14\\custom2\\"
    modeltitle = "face2"
    #opt options
    useSELU = True
    useBnorm = False

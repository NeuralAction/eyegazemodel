...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    x = tf.placeholder(tf.float32, shape=[None, dataOpen.imagesize, dataOpen.imagesize, 3], name='input_image')
    ...
    #conv
    h_pool = conv2dSingle(x, phase_train, useBnorm, [3, 3, 8])
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 10])
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 10], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 20], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 20], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 20])
    print(h_pool)
    h_pool = avg_pool_2x2(h_pool, size=shape(h_pool)[1])
    h_pool = flat(h_pool)
    print(h_pool)
    #h_pool = fc(h_pool, 32, keep_prob, phase_train)
    #print(h_pool)
    ...
    if(useRateDecay):
        ...
        decay_r = decay_ephoc * (dataSize) / bsize
    ...
    with tf.Session(config=config) as sess:
        ...
        #train loop
        for i in range(0, int(dataSize * ephoc / batch_size)):
            batch_img_open, batch_label_open = dataOpen.batch(int(round(batch_size / 2)))
            batch_img_close, batch_label_close = dataClose.batch(int(round(batch_size / 2)))
            ...
            if i !=0 and i % debugstep == 0:
                #run train acc
                fetches = sess.run([loss, mean_accuracy, y_nn[0], y_[0]], feed_dict={ x:batch_img, y_:batch_label, phase_train:False, keep_prob: testdrop })
                #acc update
                tacc = fetches[1]
                if(acc_max < tacc):
                    acc_max = tacc
                ...
                acc_count+=1
                #run test acc
                tbatch_img, tbatch_label = datatest.batch(tbsize, randomize = False)
                tfetches = sess.run([loss, mean_accuracy], feed_dict={ x:tbatch_img, y_:tbatch_label, phase_train:False, keep_prob: testdrop })
                testacc = tfetches[1]
                acc_test.append(testacc)
                #ephoc update
                e = math.floor(step/dataSize)
                if(e!=lastephoc):
                    ...
                    plt.grid(True)
                    pltname = modeltitle + " ephocs " + str(e) + "-" + str(ep) + " anglemul " + str(dataOpen.anglemul) + " lr " + str(lr) + ".png"
            ...
            #train nn
            t = sess.run([train_step], feed_dict={x: batch_img, y_: batch_label, phase_train:True, keep_prob: droprate })
            del batch_img, batch_label, t, batch_img_open, batch_img_close, batch_label_close, batch_label_open
...
def Train(save = False, loadmodel=False, ep=60, useBnorm=True, bsize=20, tbsize=20, debugStep=100, decayEphoc=4):
    dataOpen.anglemul = 1
    datatest.anglemul = dataOpen.anglemul
    lr = 0.0001
    drop = 0.75
    if(useSELU):
        drop = 0.05
...
def FreezeGraph(usecpu = False):
    ...
    with tf.Session(config=config) as sess:
        ...
        input_node_names = ["input_image", "phase_train"]
        output_node_names = ["output"]
        placeholder_type_enum = [dtypes.float32, dtypes.bool, dtypes.float32]
        placeholder_type_enum = [dtypes.float32, dtypes.bool]
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eyeclose3\\"
    modeltitle = "close"
    #opt options
    useSELU = False
    useBnorm = True
    ...
    dataOpen.imagesize = 36
    dataOpen.rotate = 45
    dataOpen.randpad = 0.25
    ...
    dataClose.randpad = 0.25

    dataSize = dataOpen.size + dataClose.size
    ...
    Train(True, loadmodel=False, ep=150, useBnorm=useBnorm, bsize=125, tbsize=125, debugStep=20, decayEphoc=5)
    FreezeGraph()

...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 8])
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 16], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 16], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 16], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 32], poolsize=1)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 32])
    print(h_pool)
    h_pool = flat(h_pool)
    print(h_pool)

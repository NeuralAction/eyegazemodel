...
def freeze_graph_with_def_protos(
  ...
  with session.Session() as sess:
    print("freeze graph: fix nodes")
    for node in sess.graph_def.node:
      if node.op == 'RefSwitch':
        print("freeze graph: refswitch to switch : " + str(node))
        node.op = 'Switch'
        for index in range(len(node.input)):
          if 'moving_' in node.input[index]:
            node.input[index] = node.input[index] + '/read'
      elif node.op == 'AssignSub':
        print("freeze graph: assignsub to sub : " + str(node))
        node.op = 'Sub'
        if 'use_locking' in node.attr: del node.attr['use_locking']
      elif node.op == 'AssignAdd':
        print("freeze graph: assignadd to add : " + str(node))
        node.op = 'Add'
        if 'use_locking' in node.attr: del node.attr['use_locking']
    if input_saver_def:
...
def main(unused_args):
  freeze_graph(FLAGS.input_graph, FLAGS.input_saver, FLAGS.input_binary,
               FLAGS.input_checkpoint, FLAGS.output_node_names, FLAGS.restore_op_name,
               FLAGS.filename_tensor_name, FLAGS.output_graph, FLAGS.clear_devices,

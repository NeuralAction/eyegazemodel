...
from vision import eyemodel_closeopen, image
import freeze_graph
...
def eval(bsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True):
    ...
    h_conv1 = conv2d(x, W_conv1, pad='SAME', stride = 1) + b_conv1
    if(useBnorm):
        h_conv1 = batch_norm(h_conv1, 20, phase_train)
    ...
    if(useBnorm):
        h_conv2 = batch_norm(h_conv2, 20, phase_train)
    h_conv2 = activate(h_conv2)
    h_pool2 = max_pool_2x2(h_conv2)
    print(h_pool2)

    #conv3
    W_conv3 = weight_variable([3, 3, 20, 20])
    b_conv3 = bias_variable([20])

    h_conv3 = conv2d(h_pool2, W_conv3) + b_conv3
    if(useBnorm):
        h_conv3 = batch_norm(h_conv3, 20, phase_train)
    ...
    #fc1
    size_fc1 = 4 * 4 * 20
    W_fc1 = weight_variable([size_fc1, 32])
    ...
    #classificant
    W_fclast = weight_variable([32, 2])
    b_fclast = bias_variable([2])
    
    h_fc2 = tf.add(tf.matmul(h_fc1, W_fclast), b_fclast)
    h_fc2 = tf.nn.softmax(h_fc2, name="output")
    y_nn = h_fc2
    ...
    with tf.Session() as sess:
        ...
        for i in range(0, int(data.size * ephoc / batch_size)):
            ...
            if i !=0 and i % debugstep == 0:
                ...
                acc_count+=1
                #run test acc
                tbatch_img, tbatch_label = datatest.batch(bsize)
...
def Train(save = False):
    ...
    lr = 0.0001
    evalacc = eval(bsize=50, ep = 3, lr = lr, debugstep=10, savepath=savedir, savemodel = save, useBnorm=True)
...
if __name__ == "__main__":
    ...
    data = eyemodel_closeopen.decodeData(["C:\\Library\\정올 2017\\Source\\NeuralAction\\Vision\\EyeExtract\\bin\\x64\\Debug\\Vision\\[2017-08-14 오후 5-33-02] eyes\\left\\",
                                            "C:\\Library\\정올 2017\\Source\\NeuralAction\\Vision\\EyeExtract\\bin\\x64\\Debug\\Vision\\[2017-08-14 오후 5-33-02] eyes\\right\\",
                                            "C:\\Library\\정올 2017\\Source\\NeuralAction\\Vision\\EyeExtract\\bin\\x64\\Debug\\Vision\\[2017-08-14 오후 5-30-01] eyes\\left\\",
                                            "C:\\Library\\정올 2017\\Source\\NeuralAction\\Vision\\EyeExtract\\bin\\x64\\Debug\\Vision\\[2017-08-14 오후 5-30-01] eyes\\right\\"], p)
    data.imagesize = 25
    ...
    datatest = eyemodel_closeopen.decodeData(["C:\\Library\\정올 2017\\Source\\NeuralAction\\Vision\\EyeExtract\\bin\\x64\\Debug\\Vision\\[2017-08-14 오후 9-37-04] eyes\\left\\"], p)
    datatest.imagesize = 25

    logdir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\log\\"
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eyeclose1\\"

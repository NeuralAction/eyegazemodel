...
def RMSE(label, predict, name=None):
    return tf.reduce_mean(tf.square(predict - label), name=name)
...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #eye
    h_pool, h_poolR = conv2dShared(x_l, x_r, phase_train, useBnorm, [3, 3, 8])
    h_pool, h_poolR = conv2dShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 12])
    h_pool, h_poolR = conv2dShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 16])
    print(h_pool)

    #face
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [7, 7, 8])
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [5, 5, 16])
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [3, 3, 32])
    print(f_pool)

    #final FC
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [1, 1, 8], poolsize=1)
    f_pool = fc(dropout(flat(f_pool), keep_prob, phase_train), 50, keep_prob, phase_train)
    h_pool = tf.concat([h_pool, h_poolR], 3)
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [1, 1, 8], poolsize=1)
    h_pool = fc(dropout(flat(h_pool), keep_prob, phase_train), 50, keep_prob, phase_train)
    fi_pool = tf.concat([h_pool , f_pool], 1)
    fi_pool = fc(fi_pool, 100, keep_prob, phase_train)
    print(fi_pool)

    #regression
    y_nn = regression(fi_pool, 2)
    loss0 = RMSE(y_nn, y_)
    """ 
    y_nn = tf.concat([y_nn, fi_pool], 1)
    y_nn = fc(y_nn, 16, keep_prob, phase_train)
    y_nn1 = regression(y_nn, 2)
    loss1 = RMSE(y_nn1, y_)

    y_nn = tf.concat([y_nn, fi_pool], 1)
    y_nn = fc(y_nn, 16, keep_prob, phase_train)
    y_nn2 = regression(y_nn, 2)
    loss2 = RMSE(y_nn2, y_)

    y_nn = tf.concat([y_nn, fi_pool], 1)
    y_nn = fc(y_nn, 16, keep_prob, phase_train)
    y_nn3 = regression(y_nn, 2, "output")
    loss3 = RMSE(y_nn3, y_)
    """
    loss = loss0
    y_nn = y_nn
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(0, step_total):
            batch_img_l, batch_img_r, batch_img_f, batch_label = data.batch(bsize, randcrop=False)
            step += bsize
            if (i !=0 and i % debugstep == 0) or i == (step_total - 1):
                ...
                #run test acc
                tbatch_img_l, tbatch_img_r, tbatch_img_f, tbatch_label = datatest.batch(tbsize, randomize = False, randcrop=False)
...
if __name__ == "__main__":
    ...
    data = eyemodel_lr2face.decodeData([
        ...
        basedir + "eyesub20\\", 
        basedir + "eyesub21\\"
        ], p, preload = dataPreLoad)
    data.imagesize = 60
    data.facesize = 60
    data.debug = False

    datatest = eyemodel_lr2face.decodeData([
        basedir + "valid1\\", 
        basedir + "valid2\\"
        ], p, preload = dataPreLoad)
    datatest.imagesize = data.imagesize
    ...
    #operations
    Train(saveCheckPoint, loadmodel=loadCheckPoint, ep=250, useBnorm=useBnorm, bsize=50, tbsize=50, debugStep=20, decayEphoc=15)
    FreezeGraph()

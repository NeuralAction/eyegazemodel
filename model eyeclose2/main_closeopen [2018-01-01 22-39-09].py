...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #conv
    h_pool = conv2dSingle(x, phase_train, useBnorm, [3, 3, 4]) #32
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 8]) #16
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 16]) #8
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 32]) #4
...
if __name__ == "__main__":
    ...
    Train(True, loadmodel=False, ep=70, useBnorm=useBnorm, bsize=50, tbsize=50, debugStep=20, decayEphoc=5)
    FreezeGraph()

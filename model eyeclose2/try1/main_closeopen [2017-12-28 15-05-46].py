...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #conv
    h_pool = conv2dSingle(x, phase_train, useBnorm, [3, 3, 16])
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 32])
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 64])
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [3, 3, 128])
    h_pool = avg_pool_2x2(h_pool, shape(h_pool)[1])
    print(h_pool)
    h_pool = flat(h_pool)
    print(h_pool)
    h_pool = fc(h_pool, 48, keep_prob, phase_train)
    print(h_pool)
...
if __name__ == "__main__":
    ...
    datatest.imagesize = 50
    
    Train(True, loadmodel=False, ep=50, useBnorm=useBnorm, bsize=50, tbsize=50, debugStep=20, decayEphoc=5)
    FreezeGraph()

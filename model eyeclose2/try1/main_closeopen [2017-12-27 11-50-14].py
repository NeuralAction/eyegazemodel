...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #conv
    h_pool = conv2dSingle(x, phase_train, useBnorm, [3, 3, 16], poolsize = 1)
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 24])
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 32])
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 48], poolsize = 1)
    h_pool = mobileLayer(h_pool, phase_train, useBnorm, [3, 3, 64])
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(0, int(data.size * ephoc / batch_size)):
            ...
            if i !=0 and i % debugstep == 0:
                ...
                acc_count+=1
                #run test acc
                tbatch_img, tbatch_label = datatest.batch(tbsize)
...
def FreezeGraph(usecpu = False):
    ...
    with tf.Session(config=config) as sess:
        ...
        input_node_names = ["input_image", "keep_prob", "phase_train"]
        output_node_names = ["output"]
        placeholder_type_enum = [dtypes.float32, dtypes.float32, dtypes.bool]
...
if __name__ == "__main__":
    ...
    datatest.imagesize = 32
    
    Train(True, loadmodel=False, ep=60, useBnorm=useBnorm, bsize=50, tbsize=50, debugStep=20, decayEphoc=5)
    FreezeGraph()

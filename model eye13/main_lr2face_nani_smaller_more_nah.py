...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #eye
    h_pool, h_poolR = conv2dShared(x_l, x_r, phase_train, useBnorm, [7, 7, 16]) # 30
    h_pool, h_poolR = conv2dShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 24], poolsize=1)
    h_pool, h_poolR = conv2dShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 24]) # 15
    h_pool, h_poolR = conv2dShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 32], poolsize=1)
    h_pool, h_poolR = conv2dShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 32]) # 8
    h_pool_8 = h_pool
    h_poolR_8 = h_poolR
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 128, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 128, phase_train, useBnorm)
    h_pool = tf.concat([h_pool, h_pool_8], 3)
    h_poolR = tf.concat([h_poolR, h_poolR_8], 3)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 160, phase_train, useBnorm, poolsize=2)
    h_pool_4 = h_pool
    h_poolR_4 = h_poolR
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 160, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 160, phase_train, useBnorm)
    h_pool = tf.concat([h_pool, h_pool_4], 3)
    h_poolR = tf.concat([h_poolR, h_poolR_4], 3)
    print(h_pool)
    #h_pool = resBlockPool(h_pool, poolsize=4)
    #h_poolR = resBlockPool(h_poolR, poolsize=4)
    #print(h_pool)
    #h_pool = tf.concat([flat(h_pool), flat(h_poolR)],1)
    #print(h_pool)

    #face
    f_pool = conv2dSingle(x_f, phase_train, useBnorm, [3, 3, 24], poolsize=1)
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [3, 3, 24]) #30
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [3, 3, 24], poolsize=1)
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [3, 3, 24]) #15
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [3, 3, 32], poolsize=1)
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [3, 3, 32]) #8
    f_pool_8 = f_pool
    f_pool = resBlock(f_pool, 128, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 128, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 128, phase_train, useBnorm)
    f_pool = tf.concat([f_pool, f_pool_8], 3)
    print(f_pool)
    f_pool = resBlock(f_pool, 160, phase_train, useBnorm, poolsize=2)
    f_pool_4 = f_pool
    f_pool = resBlock(f_pool, 160, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 160, phase_train, useBnorm)
    f_pool = tf.concat([f_pool, f_pool_4], 3)
    print(f_pool)
    #f_pool = resBlockPool(f_pool, poolsize=4)
    #print(f_pool)
    #f_pool = flat(f_pool)
    #print(f_pool)

    #final FC
    f_pool = conv2dSingle(f_pool, phase_train, useBnorm, [1, 1, 128], poolsize=1)
    #h_pool = tf.concat([h_pool, h_poolR], 3)
    h_pool = h_pool + h_poolR
    h_pool = conv2dSingle(h_pool, phase_train, useBnorm, [1, 1, 128], poolsize=1)
    #fi_pool = tf.concat([f_pool, h_pool], 3)
    #changed
    fi_pool = h_pool + f_pool
    print(fi_pool)
    #fi_pool = conv2dSingle(fi_pool, phase_train, useBnorm, [1, 1, 256], poolsize=1)
    fi_pool = resBlock(fi_pool, 128, phase_train, useBnorm)
    fi_pool = resBlock(fi_pool, 128, phase_train, useBnorm)
    fi_pool = resBlock(fi_pool, 128, phase_train, useBnorm)
    #256->128
    fi_pool = dropout(flat(fi_pool), keep_prob, phase_train)
    print(fi_pool)
    # fi_pre = tf.concat([f_pool, h_pool], 1)
    # print(fi_pre)
    # fi_pool = fc(fi_pre, 100, keep_prob, phase_train)
    # print(fi_pool)
    # fi_pool = fc(fi_pool, 64, keep_prob, phase_train)
    # print(fi_pool)
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eye14\\smn\\"
    modeltitle = "face2"
    #use selu options
    useSELU = True
    #batch norm
    ...
    #operations
    #Train(saveCheckPoint, loadmodel=loadCheckPoint, ep=250, useBnorm=useBnorm, bsize=50, tbsize=50, debugStep=10, decayEphoc=15)
    FreezeGraph()

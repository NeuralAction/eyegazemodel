...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    #eye
    h_pool, h_poolR = conv2dShared(x_l, x_r, phase_train, useBnorm, [7, 7, 24])
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 96, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 96, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 96, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 128, phase_train, useBnorm, poolsize=2)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 128, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 128, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 192, phase_train, useBnorm, poolsize=2)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 192, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 192, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 256, phase_train, useBnorm, poolsize=2)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 256, phase_train, useBnorm)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 256, phase_train, useBnorm)
    h_pool = resBlockPool(h_pool)
    h_poolR =resBlockPool(h_poolR)
    ...
    #face
    f_pool = conv2dSingle(x_f, phase_train, useBnorm, [7, 7, 32]) #64x64
    f_pool = resBlock(f_pool, 128, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 128, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 128, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 256, phase_train, useBnorm, poolsize = 2) #32x32
    f_pool = resBlock(f_pool, 256, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 256, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 384, phase_train, useBnorm, poolsize = 2) #16x16
    f_pool = resBlock(f_pool, 384, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 384, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 512, phase_train, useBnorm, poolsize = 2) #8x8
    f_pool = resBlock(f_pool, 512, phase_train, useBnorm)
    f_pool = resBlock(f_pool, 512, phase_train, useBnorm)
    f_pool = resBlockPool(f_pool) #4x4
    ...
    #final FC
    fi_pre = tf.concat([f_flat, h_flat], 1)
    ...
    # print(fi_fc1)
    fi_fc1 = dropout(fi_pre, keep_prob, training=phase_train)

    #regression
    fi_fc1_size = shape(fi_fc1)[1]
    W_fclast = fcWeight([fi_fc1_size, 2])
    b_fclast = biasWeight([2])
    
    #TODO FIXED.
    y_nn = tf.add(tf.matmul(fi_fc1, W_fclast), b_fclast, name="output")
...
if __name__ == "__main__":
    ...
    #FreezeGraph()
    ModelTest("try3\\frozen.pb", count=200, useBnorm=useBnorm, testdata=datatest)
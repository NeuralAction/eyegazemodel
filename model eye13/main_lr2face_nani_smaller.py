...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 100, phase_train, useBnorm, poolsize=2)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 100, phase_train, useBnorm, poolsize=2)
    h_pool, h_poolR = resBlockShared(h_pool, h_poolR, 100, phase_train, useBnorm)
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eye13\\smaller\\"
    modeltitle = "face2"
    #use selu options
    useSELU = False
    #batch norm

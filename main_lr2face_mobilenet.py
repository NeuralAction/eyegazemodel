...
mobileLayerToConv = False

def mobileLayer(pool, phase_train, useBnorm, weightShape, chMul = 1, poolsize = 2):
    if(mobileLayerToConv):
        pool = conv2dSingle(pool, phase_train, useBnorm, weightShape, stride = 1, poolsize = poolsize)
    else:
...
def eval(bsize=20, tbsize=20, ep = 25, lr = 1e-4, debugstep=8, savepath=None, savemodel = False, useBnorm = True, droprate = 0.5, decay_ephoc=1, decay_rate=0.8, loadmodel=False):
    ...
    h_pool, h_poolR = mobileLayerShared(h_pool, h_poolR, phase_train, useBnorm, [3, 3, 32])
    ...
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 32], poolsize = 1)
    ...
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 128])
    ...
    f_pool = mobileLayer(f_pool, phase_train, useBnorm, [3, 3, 386])
    f_pool = flat(f_pool)
    f_pool = fc(f_pool, 96, keep_prob, phase_train)
    f_pool = fc(f_pool, 48, keep_prob, phase_train)
    print(f_pool)
...
if __name__ == "__main__":
    ...
    savedir = "C:\\Users\\AinL\\Documents\\Visual Studio Code\\eyegazemodels\\model eye14\\mobile\\"
    modeltitle = "face2"
    #opt options
    mobileLayerToConv = False
    useSELU = True
